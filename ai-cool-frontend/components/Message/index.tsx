      ...
      <div className="relative z-2 border-2 border-n-3 rounded-xl overflow-hidden dark:border-n-5">
        ...
        <div className="relative flex items-center min-h-[3.5rem] px-16 text-0">
          ...
          <TextareaAutosize
            ...
            value={value}
            onChange={onChange}
            placeholder={placeholder || "Ask Ai Cool anything"}
          />
          {value === "" ? (
            <button className={`${stylesButton}`}>
              <Icon
                className="fill-n-4 transition-colors group-hover:fill-green-500"
                name="recording"
              />
            </button>
          ) : (
            <button
              onClick={handleSend}
              type="submit"
              className={`${stylesButton} bg-green-500 rounded-xl transition-colors hover:bg-primary-1/90`}
            >
